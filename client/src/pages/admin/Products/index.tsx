...
function ProductModal({ product, categories, brands, onClose }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name_mn || !form.category_id || !form.brand_id || !form.price) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto py-6 px-4"
      ...
      <div className="w-full max-w-2xl rounded-2xl shadow-modal" style={{ background: 'var(--surface-0)' }}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Жишээ: MacBook Pro 14, Galaxy S26 Ultra..."
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" value={form.price} onChange={e => set('price', e.target.value)}
                required min="0" className="input text-sm" placeholder="1990000" />
            </div>
            <div>
              ...
              <input type="number" value={form.sale_price} onChange={e => set('sale_price', e.target.value)}
                min="0" className="input text-sm" placeholder="1790000" />
            </div>
            <div>
              ...
              <input type="number" value={form.stock_quantity} onChange={e => set('stock_quantity', e.target.value)}
                min="0" className="input text-sm" placeholder="10" />
            </div>
          </div>

          {/* Статус */}
          <div>
            <label className="text-xs font-semibold mb-2 block" style={{ color: 'var(--text-secondary)' }}>Статус</label>
          ...
          {/* Тайлбар — зөвхөн нэг талбар */}
          <div>
            <label className="text-xs font-semibold mb-1.5 block" style={{ color: 'var(--text-secondary)' }}>Тайлбар</label>
            <textarea
              value={form.description_mn}
              onChange={e => handleDescMn(e.target.value)}
              rows={3}
          ...
          <div>
            ...
            {specs.length === 0 ? (
              <div className="text-center py-5 rounded-xl border border-dashed" style={{ borderColor: 'var(--border)' }}>
                <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>Үзүүлэлт нэмэгдээгүй</p>
              ...
              <>
                <div className="grid grid-cols-12 gap-2 mb-1.5 px-0.5">
                  <div className="col-span-3 text-xs" style={{ color: 'var(--text-tertiary)' }}>Бүлэг</div>
                    ...
                    <div key={i} className="grid grid-cols-12 gap-2 items-center">
                      ...
                      <div className="col-span-4">
                        <input value={spec.key} onChange={e => setSpec(i, 'key', e.target.value)}
                          placeholder="CPU, RAM..." className="input text-xs py-1.5" />
                      </div>
                      <div className="col-span-4">
                        <input value={spec.value} onChange={e => setSpec(i, 'value', e.target.value)}
                          placeholder="M3 Pro, 16GB..." className="input text-xs py-1.5" />
                      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-2xl" style={{ color: 'var(--text-primary)' }}>Бүтээгдэхүүн</h1>
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>Нийт {meta?.total || 0} бүтээгдэхүүн</p>
        </div>
            ...
            <tbody>
              {isLoading
                ...
                : products.length === 0
                  ...
                  : products.map((p: any) => (
                    <tr key={p.id} className="hover:bg-[var(--surface-1)] transition-colors"
      ...
      {showModal && (
        <ProductModal product={editProduct} categories={categories} brands={brands}

...
function CategoryModal({ cat, categories, onClose }: { cat?: any; categories: any[]; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.slug) { toast.error('Нэр, slug заавал бөглөнө'); return; }
    setLoading(true);
    try {
      if (isEdit) await adminApi.updateCategory(cat.id, form);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.name} onChange={e => { const v = e.target.value; setForm(f => ({ ...f, name: v, slug: slugify(v) })); }} required className="input text-sm" placeholder="Laptops" />
            </div>
            <div>
              <label className="text-xs font-semibold mb-1.5 block" style={{ color: 'var(--text-secondary)' }}>Нэр (Монгол)</label>
              <input value={form.name_mn} onChange={e => setForm(f => ({ ...f, name_mn: e.target.value }))} className="input text-sm" placeholder="Зөөврийн компьютер" />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold mb-1.5 block" style={{ color: 'var(--text-secondary)' }}>Slug *</label>
            <input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} required className="input text-sm font-mono" placeholder="laptops" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
              ...
              <input type="number" value={form.sort_order} onChange={e => setForm(f => ({ ...f, sort_order: Number(e.target.value) }))} className="input text-sm" min="0" />
            </div>
          </div>
          <div>
...
export default function AdminCategories() {
  ...
  const { data: categories = [], isLoading } = useQuery({ queryKey: ['categories'], queryFn: adminApi.listCategories });

  const deleteMutation = useMutation({
    mutationFn: adminApi.deleteCategory,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['categories'] }); toast.success('Устгагдлаа'); setDeleteId(null); },
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface-1)' }}>
              {['Нэр', 'Монгол нэр', 'Slug', 'Бүтээгдэхүүн', 'Дараалал', 'Үйлдэл'].map(h => (
          ...
          <tbody>
            {isLoading
              ...
              : categories.map((cat: any) => (
                <tr key={cat.id} className="hover:bg-[var(--surface-1)] transition-colors" style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td className="px-4 py-3 text-sm" style={{ color: 'var(--text-secondary)' }}>{cat.name_mn || '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => { setEditCat(cat); setShowModal(true); }} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[var(--surface-2)] transition-colors">
                        <Pencil className="w-3.5 h-3.5" style={{ color: 'var(--text-secondary)' }} />
                      </button>
                      <button onClick={() => setDeleteId(cat.id)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="card p-6 max-w-sm w-full text-center">
            ...
            <p className="text-sm mb-5" style={{ color: 'var(--text-secondary)' }}>Энэ ангилалтай холбоотой бүтээгдэхүүн байвал устгагдахгүй.</p>
      ...
      {showModal && <CategoryModal cat={editCat} categories={categories} onClose={() => { setShowModal(false); setEditCat(null); }} />}
    </div>
  );
}
